...
def main():
    ...
    comments = sentences
    responses = [sentences[0]] + sentences
    del responses[-1]
    ...
    recurrentLayerSizes = [500, 500]
    recurrentLayers = []
    
    for i in recurrentLayerSizes:
        cell = tf.nn.rnn_cell.GRUCell(i)
        recurrentLayers.append(cell)
    
    multiRNNCells = tf.nn.rnn_cell.MultiRNNCell(recurrentLayers)
    multiRNNCells = tf.nn.rnn_cell.DropoutWrapper(multiRNNCells, input_keep_prob=0.7)
    ...
    recurrentInput = tf.reshape(XW, [batch, sentence_length, embeddingD])
